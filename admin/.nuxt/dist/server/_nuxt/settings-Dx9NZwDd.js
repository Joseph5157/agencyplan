...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm grid gap-4 md:grid-cols-2"><div><label class="mb-1 block text-sm font-medium text-slate-700">School Name (Read-only)</label><input${ssrRenderAttr("value", unref(school)?.name || "")} disabled type="text" class="w-full rounded-md border border-slate-300 bg-slate-100 px-3 py-2 text-sm text-slate-600"></div><div><label class="mb-1 block text-sm font-medium text-slate-700">Template (Read-only)</label><input${ssrRenderAttr("value", unref(school)?.template || "")} disabled type="text" class="w-full rounded-md border border-slate-300 bg-slate-100 px-3 py-2 text-sm text-slate-600"><p class="mt-1 text-xs text-amber-700">Contact Syed Websites to change template.</p></div><div class="md:col-span-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600"> Admin cannot change template, layout, colors, fonts, CSS, or section order from this page. </div></div><form class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm grid gap-4 md:grid-cols-2"><div class="space-y-2"><label class="block text-sm font-medium text-slate-700">Logo</label>`);
...
//# sourceMappingURL=settings-Dx9NZwDd.js.map
